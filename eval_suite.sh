mkdir -p etc
python eval_from_json.py $QBOTIO_SEARCH_API -i sample_corpus/all-qa.json -o etc/nltk_eval.json -v
python eval_from_json.py $QBOTIO_SEARCH_API -i sample_corpus/all-qa.json -o etc/passthrough_eval.json -v --passthrough
